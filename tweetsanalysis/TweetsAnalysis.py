...
def text_sentiment_analysis(text):
    ...
    payload = "key=afaf04e25bc92f7d3981fbc4790fa05f&lang=en&txt=%s&model=general" % {
        text}
...
def topic_extraction(txt):
    ...
    payload = "key=afaf04e25bc92f7d3981fbc4790fa05f&lang=en&txt=%s&tt=a" % {
        txt}
    headers = {'content-type': 'application/x-www-form-urlencoded'}
    response = requests.request("POST", url, data=payload, headers=headers)
...
results = db.view(
    'tweets_analysis/filter_tweets_with_coordinates_inside_melbourne', limit=1000)
for row in results:
    # get Suburb
